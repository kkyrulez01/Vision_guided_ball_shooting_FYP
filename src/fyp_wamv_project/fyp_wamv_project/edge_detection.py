...
def detect_target(frame):
    ...
    for cnt in contours:
        ...
        if area > 500:
            ...
            # targets_centre.append((x+w//2, y+h//2)) # Append the centre of the shape

            rect = cv2.minAreaRect(cnt)
            box = cv2.boxPoints(rect)
            box = np.int0(box)
            cv2.drawContours(contours_frame, [box], 0, (255,0,255), 2)
            center_x = int(rect[0][0])
            center_y = int(rect[0][1])
            targets_centre.append((center_x, center_y))

...
def main():
    ...
    disparity = DepthMap(left_camera, right_camera)
    ...
    # # Compute disparity maps
    # disp_l = left_matcher.compute(left_region, right_region).astype(np.float32) / 16.0
    # disp_r = right_matcher.compute(right_region, left_region).astype(np.float32) / 16.0
    # filtered_disp = wls_filter.filter(disp_l, left_region, disparity_map_right=disp_r)
    # filtered_disp = filtered_disp[:,16*8:-1]

    # Plot outputs
    # fig, (ax1, ax2) = plt.subplots(1,2, figsize = (15,10))
    
    # ax1.imshow(left_region, 'gray')
